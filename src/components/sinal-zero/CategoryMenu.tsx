  ...
  return (
    <div className="flex min-w-0 shrink-0 items-center gap-1.5 overflow-visible">
      ...
      <Popover>
        ...
        <PopoverContent align="start" sideOffset={8} className="z-[5000] w-[min(92vw,300px)] max-w-[calc(100vw-24px)] border-primary/70 bg-card/98 p-2.5 shadow-[0_0_0_1px_rgba(255,173,0,.18),0_20px_55px_-28px_rgba(255,173,0,.65)] backdrop-blur-xl">
          <div className="mb-2.5 flex items-center justify-between gap-2 px-1"><div><span className="block text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Categorias</span><span className="text-[10px] text-muted-foreground">Escolha uma ou mais áreas</span></div><button type="button" onClick={() => onChange(value.length === ALL_KEYS.length ? [] : ALL_KEYS)} className="rounded-md border border-primary/60 px-2 py-1 text-[10px] font-semibold text-primary transition-colors hover:border-primary hover:bg-primary/10">{value.length === ALL_KEYS.length ? "Limpar" : "Todas"}</button></div>
          <div className="grid max-h-80 grid-cols-1 gap-1 overflow-y-auto pr-1 sm:grid-cols-2">{ALL_KEYS.map((key) => { const active = value.includes(key); return <button key={key} type="button" onClick={() => toggle(key)} className={cn("flex min-h-9 items-center justify-between gap-2 rounded-lg border px-2.5 py-2 text-left text-[11px] font-medium transition-colors duration-150", active ? "border-primary/60 bg-primary/10 text-primary shadow-[0_0_16px_-12px_rgba(255,173,0,.9)]" : "border-primary/30 text-muted-foreground hover:border-primary/60 hover:bg-primary/5 hover:text-foreground")}>{CATEGORY_LABELS[key]}{active && <Check className="h-3.5 w-3.5 shrink-0" />}</button>; })}</div>
          <p className="mt-2 border-t border-primary/40 pt-2 text-center text-[9px] leading-relaxed text-muted-foreground">Sem seleção = buscar todas as categorias disponíveis.</p>
        </PopoverContent>
      </Popover>
    </div>
  );
