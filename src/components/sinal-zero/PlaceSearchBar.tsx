...
export function PlaceSearchBar({ onPick, scanning, currentLabel }: PlaceSearchBarProps) {
  ...
  const pick = async (suggestion: SearchSuggestion) => {
    if (suggestion.kind === "state") { const state = suggestion.value; const place: PlaceSuggestion = { label: `Estado de ${state.name}, Brasil`, shortLabel: `${state.name}, ${state.code}`, lat: state.lat, lon: state.lon, boundingBox: null, scope: "state", stateCode: state.code }; pickedLabelRef.current = place.shortLabel; setSuggestions([]); setValue(place.shortLabel); setOpen(false); onPick(place); return; }
    ...
    finally { const remaining = Math.max(0, MIN_SEARCH_FEEDBACK_MS - (performance.now() - startedAt)); if (remaining) await wait(remaining); setLoading(false); }
  };
