...
export function classifySignals(tags: Record<string, string>, contact?: EstablishmentContact) {
  const resolvedContact = contact ?? buildContact(tags);
  const website = Boolean(resolvedContact.websiteUrl);
  const instagram = Boolean(resolvedContact.instagramUrl);
  ...
  const whatsapp = Boolean(resolvedContact.whatsappValid);
...
function normalizeUrl(value: string | null): string | null {
  ...
  try {
    ...
    return url.hostname.includes(".") && !url.username && !url.password ? url.href : null;
  } catch {
    return null;
  }
}
...
function instagramFromValue(value: string | null) {
  ...
  if (/instagram\.com\//i.test(value)) {
    try {
      ...
      if (!/^(www\.)?instagram\.com$/i.test(url.hostname)) return { handle: null, url: null };
  ...
  if (
    ...
    ["p", "reel", "reels", "stories", "explore", "accounts"].includes(handle.toLowerCase())
  )
    return { handle: null, url: null };
  return handle.length < 2
    ? { handle: null, url: null }
    : { handle: `@${handle}`, url: `https://instagram.com/${handle}` };
}

export function toWhatsappNumber(raw: string | null): string | null {
  if (!raw) return null;
  if (/^(https?:\/\/)?(wa\.me|api\.whatsapp\.com)\//i.test(raw.trim())) {
    try {
      const url = new URL(normalizeUrl(raw) ?? "");
      const number =
        url.hostname === "wa.me" ? url.pathname.slice(1) : url.searchParams.get("phone");
      return number && /^\d{11,15}$/.test(number) ? toWhatsappNumber(`+${number}`) : null;
    } catch {
      return null;
    }
  }
  ...
  if (digits.startsWith("55") && (digits.length === 12 || digits.length === 13)) {
    ...
    if (ddd >= 11 && ddd <= 99 && subscriber.length === 9 && subscriber.startsWith("9"))
      return digits;
    return null;
  }
...
function extractRating(tags: Record<string, string>): number | null {
  ...
  return Number.isFinite(value) && value > 0 ? Math.min(5, Math.round(value * 10) / 10) : null;
}
function extractPriceLevel(tags: Record<string, string>): 1 | 2 | 3 | null {
  ...
  if (/^\$+$/.test(value) || /^€+$/.test(value))
    return Math.min(3, Math.max(1, (value.match(/[$€]/g) ?? []).length)) as 1 | 2 | 3;
...
function resolveCategory(tags: Record<string, string>): { label: string; key: CategoryKey | null } {
  const matches = (Object.keys(CATEGORIES) as CategoryKey[]).filter((candidate) =>
    CATEGORIES[candidate].filters.some(
      (filter) =>
        tags[filter.key] !== undefined && filter.values.includes(tags[filter.key] as string),
    ),
  );
  const key = matches[0] ?? null;
...
export function processOverpassResults(
  ...
  for (const element of elements) {
    const tags = element.tags ?? {},
      name = getTag(tags, ["name", "official_name"]),
      center =
        element.center ??
    ...
    if (
      categorySet.size > 0 &&
      !categories.some((key) =>
        CATEGORIES[key].filters.some((filter) => filter.values.includes(tags[filter.key] ?? "")),
      )
    )
      continue;
    ...
    const classification = classifySignals(tags, contact),
    ...
    results.push({
      ...
      knownBrand: classification.knownBrand,
      rating,
      priceLevel,
