...
async function queryStateArea(stateCode: string, categories: CategoryKey[]) {
  ...
  const primary = await queryOverpass(buildStateOverpassQuery(stateCode, categories), { requestTimeoutMs: 30000, totalTimeoutMs: 34000 });
  ...
  return queryOverpass(buildAroundQuery(state.lat, state.lon, categories, 30000), { requestTimeoutMs: 12000, totalTimeoutMs: 16000 });
}
