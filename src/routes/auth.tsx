...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data: sessionData }) => { if (sessionData.session) void navigate({ to: "/" }); });
        ...
        <div className="glass-panel rounded-[2rem] border border-border/70 p-7 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-8 auth-card-enter">
          {step === "credentials" && <>
            ...
            <form onSubmit={submitCredentials} className="space-y-4">
              ...
              <label className="block text-sm font-medium auth-reveal">Senha
                <div className="relative mt-1.5 h-12">
                  ...
                  <button type="button" aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} onClick={() => setShowPassword((value) => !value)} className="absolute right-2 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg p-0 leading-none text-muted-foreground transition-colors hover:bg-muted hover:text-foreground active:scale-90" style={{ margin: 0 }}>
