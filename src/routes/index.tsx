    ...
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground lg:h-screen">
      ...
      <main className="min-h-0 flex-1 overflow-auto px-3 py-4 sm:px-5 lg:px-7 lg:py-5">
        <div className="mx-auto flex min-h-full w-full max-w-[1500px] flex-col gap-4">
          ...
          <section className="panel-enter flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-card/55 shadow-xl shadow-black/10 backdrop-blur-xl">
            ...
            <div className={`relative min-h-[320px] flex-1 ${isBusy ? "p-0" : "p-3 sm:p-4"}`}>
              {isBusy && <div className="loading-state-enter absolute inset-0 flex h-full w-full flex-col items-center justify-center text-center" role="status" aria-live="polite"><AreaSearchRadar verifying={verifyingPresence} /><span className="loading-state-title">{scanning ? "Pesquisando a área..." : "Verificando presença digital..."}</span><span className="loading-state-subtitle">Aguarde enquanto os estabelecimentos são qualificados.</span></div>}
