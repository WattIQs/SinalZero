...
function ratingMatchesFilter(rating: number | null, filters: string[]) {
  ...
  return filters.some(
    (filter) =>
      filter !== "unrated" &&
      Number.isFinite(Number.parseInt(filter, 10)) &&
      Math.round(rating) === Number.parseInt(filter, 10),
  );
}
...
function hasWebsite(lead: Establishment) {
...
function matchesContactFilter(lead: Establishment, filters: ContactFilter[]) {
  if (!filters.length) return true;
  return filters.some((filter) =>
    filter === "whatsapp" ? lead.contact.whatsappValid : Boolean(lead.contact.instagramUrl),
  );
}

function applyPresenceFilters(
  leads: Establishment[],
  ...
  return leads.filter(
    (lead) =>
      matchesSignalFilter(lead, signalFilters) &&
      matchesContactFilter(lead, contactFilters) &&
      (!noWebsite || !hasWebsite(lead)),
  );
}
...
function Index() {
  ...
  const [scanTarget, setScanTarget] = useState<PlaceSuggestion | null>(null);
  const [scanCategories, setScanCategories] = useState<CategoryKey[]>([]);
  ...
  const applyCurrentFilters = (
    ...
    selectedContacts = contactFilters,
    noWebsite = noWebsiteOnly,
  ) => {
    const next = applyPresenceFilters(
      filterByCategory(source, selectedCategories),
      selectedSignals,
      selectedContacts,
      noWebsite,
    );
    setResults(next);
    setError(
      next.length === 0 && source.length > 0 ? "Nenhum resultado para os filtros atuais." : null,
    );
  };

  const enrichPresence = async (
    source: Establishment[],
    scanId = scanIdRef.current,
    filterId = filterRunRef.current,
  ): Promise<Establishment[] | null> => {
    const isCurrent = () => scanId === scanIdRef.current && filterId === filterRunRef.current;
    ...
    try {
      ...
      if (!externalVerificationAvailable)
        setNotice(
          "Contatos e presença digital baseados nos dados cadastrados na fonte; verificação web indisponível.",
        );
      return source.map((lead) => verificationCacheRef.current.get(lead.id) ?? lead);
    } catch (err) {
      if (!isCurrent()) return null;
      setNotice(
        "Não foi possível completar a verificação digital. Os estabelecimentos encontrados foram preservados.",
      );
      return source.map((lead) => verificationCacheRef.current.get(lead.id) ?? lead);
    } finally {
      if (isCurrent()) setVerifyingPresence(false);
    }
  };

  const refreshPresenceFilters = async (
    source: Establishment[],
    selectedCategories = categories,
    selectedSignals = signalFilters,
    selectedContacts = contactFilters,
    ...
    const needsVerification =
      selectedSignals.length > 0 || selectedContacts.length > 0 || noWebsite;
    if (!needsVerification) {
      setVerifyingPresence(false);
      if (runId === filterRunRef.current)
        applyCurrentFilters(
          source,
          selectedCategories,
          selectedSignals,
  ...
  const runScan = async (target: PlaceSuggestion, page = 0) => {
    const requestedCategories = page ? scanCategories : categories;
    ...
    try {
      const data = await searchPlaces({
        data: {
          area: target.boundingBox ?? {
            south: target.lat - 0.025,
            north: target.lat + 0.025,
            west: target.lon - 0.03,
            east: target.lon + 0.03,
          },
          categories: requestedCategories,
          stateCode: target.scope === "state" ? target.stateCode : undefined,
          page,
        },
      });
      if (scanId !== scanIdRef.current) return;
      const fresh = processOverpassResults(data.elements, requestedCategories);
      const processed = [
        ...new Map([...(page ? allResults : []), ...fresh].map((lead) => [lead.id, lead])).values(),
      ];
      setNextPage(data.nextPage);
      if (data.limited)
        setNotice(
          "Este lote atingiu o limite da fonte. Os resultados são uma amostra; refine a área ou a categoria para ampliar a cobertura.",
        );
      ...
      if (!processed.length)
        setError(
          data.nextPage !== null
            ? "Nenhum resultado neste lote. Continue a busca nas próximas categorias."
            : "Nenhum estabelecimento encontrado nos lotes consultados para esta área.",
        );
    } catch (err) {
  ...
  const handlePickPlace = (target: PlaceSuggestion | null) => {
    setPlace(target);
    setError(null);
  };
  ...
  const handleCategoriesChange = (next: CategoryKey[]) => {
    ...
    if (allResults.length)
      void refreshPresenceFilters(allResults, next, signalFilters, contactFilters, noWebsiteOnly);
  };
  const handleToggleSave = (lead: Establishment) => {
    if (isLeadSaved(lead.id)) {
      void removeLead(lead.id).then((persisted) => {
        ...
        if (!persisted)
          setError(
            "Lead removido deste dispositivo; a remoção será sincronizada quando a conexão voltar.",
          );
      });
      setSavedLeads(getSavedLeads());
      return;
    ...
    void saveOperation.then(({ persisted }) => {
      ...
      if (!persisted)
        setError(
          "Lead salvo neste dispositivo; a sincronização com sua conta será repetida automaticamente.",
        );
    });
  };
  const handleSignalFiltersChange = (next: SignalFilter[]) => {
    ...
    if (allResults.length)
      void refreshPresenceFilters(allResults, categories, next, contactFilters, noWebsiteOnly);
  };
  const handleContactFiltersChange = (next: ContactFilter[]) => {
    setContactFilters(next);
    if (allResults.length)
      void refreshPresenceFilters(allResults, categories, signalFilters, next, noWebsiteOnly);
  };
  const handleNoWebsiteChange = (enabled: boolean) => {
    setNoWebsiteOnly(enabled);
    if (allResults.length)
      void refreshPresenceFilters(allResults, categories, signalFilters, contactFilters, enabled);
  };

  const visibleResults = useMemo(() => {
    let list = applyPresenceFilters(
      results.filter((lead) => categoryMatches(lead, categories)),
      signalFilters,
      contactFilters,
      noWebsiteOnly,
    ).filter((lead) => ratingMatchesFilter(lead.rating, ratingFilters));
    if (priceFilter !== "any")
      list = list.filter((lead) => lead.priceLevel === Number.parseInt(priceFilter, 10));
    const sorted = [...list];
    sorted.sort((a, b) => {
      switch (sortKey) {
        case "rating_desc":
  ...
  useEffect(() => {
    setRenderedResultCount(RESULTS_PAGE_SIZE);
  }, [
    results,
    categories,
    ratingFilters,
    ...
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground lg:h-screen">
      <header className="relative z-[3000] shrink-0 border-b border-border bg-card/95 px-3 py-2.5 shadow-lg shadow-black/10 backdrop-blur-xl sm:px-5 lg:px-7">
        <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-2.5 lg:flex-row lg:items-center lg:gap-4">
          <div className="brand-lockup flex shrink-0 items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-primary/25 bg-primary/10 text-primary shadow-[0_0_24px_-10px_var(--color-primary)]">
              <Radar className="h-5 w-5" />
            </div>
            <div className="leading-none">
              <div className="brand-title text-sm font-bold tracking-tight">
                Sinal <span className="text-gradient-signal">Zero</span>
              </div>
              <div className="brand-tagline mt-1 text-[9px] font-medium uppercase tracking-[0.18em] text-muted-foreground">
          ...
          <div className="flex shrink-0 items-center gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            <FiltersMenu
              ...
              onSignalFiltersChange={handleSignalFiltersChange}
              contactFilters={contactFilters}
              onContactFiltersChange={handleContactFiltersChange}
              noWebsiteOnly={noWebsiteOnly}
              onNoWebsiteOnlyChange={handleNoWebsiteChange}
              sortKey={sortKey}
            ...
            <SavedLeadsDrawer
              leads={savedLeads}
              onRemove={(id) => {
                void removeLead(id).then(() => setSavedLeads(getSavedLeads()));
              }}
              onSync={setSavedLeads}
            />
          </div>
        </div>
      </header>
      ...
      <main className="flex min-h-0 flex-1 flex-col overflow-auto px-3 py-4 sm:px-5 lg:px-7 lg:py-5">
        <div className="mx-auto flex min-h-0 w-full max-w-[1500px] flex-1 flex-col gap-4">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div className="min-w-0">
                ...
                <h1 className="mt-1.5 text-xl font-bold tracking-tight sm:text-2xl">
                  Encontre empresas com potencial
                </h1>
                <p className="mt-1 max-w-2xl text-xs leading-relaxed text-muted-foreground">
                  Escolha a região, ajuste os filtros e encontre empresas que façam sentido para a
                  sua abordagem.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                <div className="rounded-xl border border-border/70 bg-background/70 px-4 py-3 text-center">
                  <div className="text-lg font-bold text-foreground">{visibleResults.length}</div>
                  <div className="text-[9px] uppercase tracking-wider text-muted-foreground">
                    Resultados
                  </div>
                </div>
                <div className="rounded-xl border border-border/70 bg-background/70 px-4 py-3 text-center">
                  <div className="text-lg font-bold text-signal-zero">{savedLeads.length}</div>
                  <div className="text-[9px] uppercase tracking-wider text-muted-foreground">
                    Salvos
                  </div>
                </div>
                <div className="hidden rounded-xl border border-border/70 bg-background/70 px-4 py-3 text-center sm:block">
                  <div className="text-lg font-bold text-cyan">{categories.length || "Todas"}</div>
                  <div className="text-[9px] uppercase tracking-wider text-muted-foreground">
                    Categorias
                  </div>
                </div>
          ...
          <section className="panel-enter relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-card/55 shadow-xl shadow-black/10 backdrop-blur-xl">
            <div className="shrink-0 flex flex-col gap-2 border-b border-border/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
              <div>
                <h2 className="text-sm font-semibold">
                  {signalTitle}
                  {contactFilters.length
                    ? ` · ${contactFilters.map((item) => (item === "whatsapp" ? "WhatsApp" : "Instagram")).join(" + ")}`
                    : ""}
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full border border-border bg-background/70 px-2.5 py-1 text-[10px] font-medium text-muted-foreground">
                  {visibleResults.length} encontrados
                </span>
                {isBusy && (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/8 px-2.5 py-1 text-[10px] font-medium text-primary">
            ...
            <div className="relative min-h-[320px] flex-1 p-3 sm:p-4">
              {notice && (
                <p role="status" className="mb-3 rounded-lg border border-primary/30 p-3 text-xs">
                  {notice}
                </p>
              )}
              {error && visibleResults.length > 0 && (
                <p role="alert" className="mb-3 rounded-lg border border-primary/30 p-3 text-xs">
                  {error}
                </p>
              )}
              {!isBusy && nextPage !== null && scanTarget && (
                <div className="mb-3 flex flex-wrap items-center gap-3">
                  <button
                    type="button"
                    className="rounded-lg border border-primary/45 px-4 py-2 text-xs"
                    onClick={() => void runScan(scanTarget, nextPage)}
                  >
                    Buscar mais categorias na área
                  </button>
                  <span className="text-xs">
                    {allResults.length} carregados · cobertura parcial da fonte
                  </span>
                </div>
              )}
              {!isBusy && visibleResults.length === 0 && (
                <div className="flex min-h-full flex-col items-center justify-center rounded-xl bg-background/25 p-8 text-center">
                  ...
                  <h3 className="mt-4 text-sm font-semibold">
                    {error ?? "Comece pela região que você quer trabalhar"}
                  </h3>
                  <p className="mt-1 max-w-md text-xs leading-relaxed text-muted-foreground">
                    {error
                      ? "Escolha outra área ou ajuste os filtros para uma nova varredura."
                      : "Pesquise uma cidade, bairro ou região acima e clique em Varrer área."}
                  </p>
                </div>
              )}
              {!isBusy && visibleResults.length > 0 && (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    {renderedResults.map((item, index) => (
                      <PlaceRow
                        ...
                        saved={savedLeadIds.has(item.id)}
                        animationDelay={Math.min(index, 14) * 55}
                        onSelect={() => setSelectedId(item.id)}
                        onToggleSave={handleToggleSave}
                      />
                    ))}
                  </div>
                  {renderedResults.length < visibleResults.length && (
                    <div className="flex justify-center pt-5">
                      <button
                        type="button"
                        onClick={() => setRenderedResultCount((count) => count + RESULTS_PAGE_SIZE)}
                        className="rounded-full border border-primary/45 bg-primary/8 px-4 py-2 text-xs font-semibold text-primary transition hover:border-primary hover:bg-primary/15"
                      >
                        Mostrar mais ({renderedResults.length} de {visibleResults.length})
                      </button>
                    </div>
                  )}
                </>
              ...
              {isBusy && (
                <div
                  className="loading-state-enter absolute inset-0 z-10 flex h-full w-full flex-col items-center justify-center overflow-hidden bg-card/98 p-6 text-center"
                  role="status"
                  aria-live="polite"
                >
                  <AreaSearchRadar verifying={verifyingPresence} />
                  <span className="loading-state-title relative z-[2]">
                    {scanning ? "Pesquisando a área..." : "Verificando presença digital..."}
                  </span>
                  <span className="loading-state-subtitle relative z-[2]">
                    Aguarde enquanto os estabelecimentos são qualificados.
                  </span>
                </div>
              )}
            </div>
